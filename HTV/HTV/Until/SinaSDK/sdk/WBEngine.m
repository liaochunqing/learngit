...
#define kWBURLSchemePrefix              @"SINA_"

#define kWBKeychainServiceNameSuffix    @"_SinaWeiBoServiceName"
#define kWBKeychainUserID               @"SinaWeiBoUserID"
#define kWBKeychainAccessToken          @"SinaWeiBoAccessToken"
#define kWBKeychainExpireTime           @"SinaWeiBoExpireTime"
                 ...
                 httpHeaderFields:(NSDictionary *)httpHeaderFields
{
    ...
    if ([methodName isEqualToString:@"statuses/user_timeline.json"]) {
        [request disconnect];
        
        self.request = [WBRequest requestWithAccessToken:nil
...
- (void)sendWeiBoWithText:(NSString *)text image:(UIImage *)image
{
    ...
    if (image)
    {
        ...
        [self loadRequestWithMethodName:@"statuses/upload.json"
                             httpMethod:@"POST"
                                 params:params
                           postDataType:kSinaWBRequestPostDataTypeMultipart
...
#pragma mark - SinaWBRequestDelegate Methods

- (void)request:(WBRequest *)arequest didFinishLoadingWithResult:(id)result
{
    
    if ([[arequest url] isEqualToString:@"https://api.weibo.com/2/friendships/create.json"]) {
        if ([delegate respondsToSelector:@selector(engine:observeSuccess:)]) {
            [delegate engine:self observeSuccess:result];
        }
    }
    ...
    if ([delegate respondsToSelector:@selector(engine:requestDidSucceedWithResult:request:)]) {
        [delegate engine:self requestDidSucceedWithResult:result request:arequest];
    }
}

- (void)request:(WBRequest *)arequest didFailWithError:(NSError *)error
{
    if ([[arequest url] isEqualToString:@"https://api.weibo.com/2/friendships/create.json"]) {
        if ([[error.userInfo valueForKey:@"error"] isEqualToString:@"already followed"]) {
            if ([delegate respondsToSelector:@selector(engine:observeSuccess:)]) {
                [delegate engine:self observeSuccess:error];
            }
        }
    ...
    if ([delegate respondsToSelector:@selector(engine:requestDidFailWithError:request:)]) {
        [delegate engine:self requestDidFailWithError:error request:arequest];
    }
}
